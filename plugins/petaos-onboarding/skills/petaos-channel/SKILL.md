---
name: petaos-channel
description: 在 PETAOS 现有网站引导和办理渠道中台任务，用于客服主管、渠道运营的操作培训、页面定位和业务结果核验。
---
...
https://petaos.tech/channel/dashboard；使用当前授权会话，或官网 Console → PETAOS Apps → 渠道中台。

## 前置条件

插件版先读 [使用边界与验证状态](../petaos-start/references/使用边界与验证状态.md)，了解授权范围及历史报错的较新复核结果；具体页面以当前会话为准。

明确渠道账号、群组、品牌与允许处理的数据；账号/套餐具备所需能力。
...
4. 规则启用、群发、Rich Menu 发布、日报推送、死信重放可能真实发消息。测试台是否有外部副作用以当前说明为准。
...
证据版本：2026-09-14；当前可见 22 个菜单/工作台入口，生产业务未提交验证。
