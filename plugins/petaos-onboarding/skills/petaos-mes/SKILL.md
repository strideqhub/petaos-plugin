...
https://petaos.tech/mes/dashboard；使用当前授权会话，或官网 Console → PETAOS Apps → MES 系统。

## 前置条件

插件版先读 [使用边界与验证状态](../petaos-start/references/使用边界与验证状态.md)，了解授权范围及历史报错的较新复核结果；具体页面以当前会话为准。

物料、单位、车间、工作站、工序、工艺与班组已维护；有生产或质检权限。
...
4. 工单、排产、领料、报工、质检和入库各有状态。报工数量不等于合格入库数量；真实设备/工厂动作不因界面保存而视作完成。
...
证据版本：2026-09-14；当前可见 60 个菜单/工作台入口，生产业务未提交验证。
